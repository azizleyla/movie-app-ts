...
const Reviews = ({movieItem}:{movieItem?:IMovie}) => {
  ...
  return (
    <div className='flex gap-5 flex-col items-start  my-10'>
      ...
      <div>
        {results?.length > 0 ? results?.slice(0,2)?.map((result:IReview) =>(
          <ReviewItem result={result}/>
        ))  :    <p className='text-white border-b-[1px]'>We don't have any reviews for {movieItem?.original_title} 
        </p>}
        {results?.length > 0 && <button onClick={handleNavigate} className='text-white underline'>Read All Reviews</button>
        }
        </div>
